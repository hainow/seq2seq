...
class TFRecordInputPipeline(InputPipeline):
  ...
  def make_data_provider(self, **kwargs):
    ...
    keys_to_features = {
      # ["shape", "dtype", "default_value"] - TFRecord features spec
    ...
    # generalization of tf.TFRecordReader().read() and tf.decode_raw()
    # all of decoded features will go to a dictionary items_to_handlers
...
class SimpleTextFileInputPipeline(InputPipeline):
  """An input pipeline that reads a TFRecords containing both source
  and target sequences.
  ...
  def make_data_provider(self, **kwargs):
    # paths = []
    # labels = []
    # with open(self.params["file_and_label_list"]) as f:
    #   for line in f:
    ...
    # dataset = tf.contrib.data.Dataset.from_tensor_slices((paths, labels))
    decoder_source = split_tokens_decoder.SplitTokensDecoder(
      tokens_feature_name="tokens",
      length_feature_name="length",
      append_token=None,
      delimiter=self.params["file_delimiter"])
  ...
  @property
  def feature_keys(self):
    return set(["file_and_label_list"])
