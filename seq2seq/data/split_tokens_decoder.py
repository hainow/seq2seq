...
class SplitTokensDecoder(data_decoder.DataDecoder):
  ...
  def decode(self, data, items):
    """
    Decode data into tokens and return the corresponding tokens w.r.t items 
    
    NOTE: Items here is either 
          (1) self.tokens_feature_name, or
          (2) self.length_feature_name, or 
          (3) both. 

    Args:
      data: encoded data to be decoded, of type `string`
      items: see NOTE above  
...
class DataTextFileDecoder(data_decoder.DataDecoder):
  """A DataProvider that splits a string tensor into individual tokens and
  returns the tokens and the length.
  Optionally prepends or appends special tokens.
  ...
  def decode(self, data, items):
    ...
    decoded_items[self.label] = tokens[1]
    decoded_items[self.path] = tokens[0]
    return [decoded_items[_] for _ in items]
